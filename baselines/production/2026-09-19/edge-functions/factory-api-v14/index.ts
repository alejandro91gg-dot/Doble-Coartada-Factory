...
function rpcError(e:any){const m=String(e?.message??e??"RPC_FAILED");const known=["AUTHORITY_NOT_FOUND","AUTHORITY_NOT_STAGED","SOURCE_DOCUMENT_REQUIRED","SOURCE_DOCUMENT_MISMATCH","SUPPORTING_GATE_NOT_PASS","AUTHORITY_NOT_VALIDATED","CURRENT_AUTHORITY_EXISTS","AUTHORITY_NOT_AUTHORITATIVE","AUTHORITY_NOT_CURRENT","SUPERSEDE_REASON_REQUIRED","AUTHORITY_NOT_CURRENT_STATE"];for(const k of known)if(m.includes(k))return k;return "AUTHORITY_TRANSITION_FAILED"}
function codeFor(e:string){if(e==="AUTHORITY_NOT_FOUND")return 404;if(["AUTHORITY_NOT_STAGED","AUTHORITY_NOT_VALIDATED","AUTHORITY_NOT_AUTHORITATIVE","AUTHORITY_NOT_CURRENT_STATE","AUTHORITY_NOT_CURRENT","CURRENT_AUTHORITY_EXISTS","SOURCE_DOCUMENT_REQUIRED","SOURCE_DOCUMENT_MISMATCH","SUPPORTING_GATE_NOT_PASS","SUPERSEDE_REASON_REQUIRED"].includes(e))return 409;return 400}
...
Deno.serve(async(req:Request)=>{
  ...
  if(req.method==="GET"&&p==="/health")return R({ok:true,service:"doble-coartada-factory-api",version:"1.4.1",gateway:"factory-api-v14",authority_registry:true,action_file_delivery:true});
