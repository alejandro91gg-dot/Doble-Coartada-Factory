---
name: chronology-auditor
description: Audits temporal consistency in the active case across REAL, CLAIMED, and PROVABLE timelines. Use whenever times, durations, windows, sequence, travel intervals, timestamps, or time-based deductions change.
---
...
1. Inventory exact timestamps, approximate/bounded times, durations, before/after relations, creation/discovery times, access windows, travel intervals, forensic windows, document times, and visible clock/screen times.
2. Build separate REAL, CLAIMED, and PROVABLE temporal graphs.
3. Preserve source precision: never turn approximate time into exact time without authority.
4. Record source, certainty/tolerance, actor/location, and dependents for each temporal fact.
5. Recalculate all affected duration/window arithmetic.
6. Check impossible overlaps, reversed order, missing transitions, and unsupported precision.
7. Trace changes through alibis, evidence, constraints, documents, phase reveals, and evidentiary visuals.
8. Require `alibi-auditor` when feasible whereabouts change.
9. Require `evidence-dependency-auditor` when proof/exclusion meaning changes.

## Output

`Temporal ID | Old value | New/proposed value | Timeline layer | Source | Direct dependents | Transitive dependents | Required action | Status`

Conclude with contradictions, arithmetic failures, unsupported precision, required re-tests, and `CHRONOLOGY AUDIT PASS` or `FAIL`.
