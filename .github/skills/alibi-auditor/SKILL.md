---
name: alibi-auditor
description: Audits alibis, whereabouts, access, route feasibility, witness support, and suspect exclusion in the active case. Use whenever claims, locations, opportunity windows, routes, access conditions, or relevant times change.
---
...
1. Separate claimed, real, and provable location for each relevant person.
2. Record access capability, travel feasibility, supporting sources, contradicting sources, and uncertainty.
3. Map each alibi against the incident/forensic window.
4. Distinguish plausible alibi from objectively established alibi.
5. Recalculate travel/access feasibility only from authorized inputs.
6. Test whether the same objective evidence also fits another suspect.
7. Trace changes into chronology, evidence meaning, suspect matrix, final exclusions, documents, and reveal dependencies.
8. Use `chronology-auditor` and `evidence-dependency-auditor` when their domains are touched.

## Output

`Person ID | Claim | REAL | PROVABLE | Access | Feasible? | Supporting evidence | Weakening evidence | Final exclusion dependency | Impact`

Flag circular/self-supported alibis, hidden-author-knowledge dependencies, alternate feasible routes, late exclusions, and canon conflicts.

Conclude `ALIBI AUDIT PASS` or `FAIL`.
