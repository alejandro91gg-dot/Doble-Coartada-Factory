---
name: investigation-fairness
description: Audits the active case for fair-play solvability. Use before canon lock, G6, G10/G11, or after changes to evidence, reveal order, exclusions, red herrings, or final proof.
---
...
- culprit not demonstrable from delivered evidence;
- viable innocents not reasonably excludable;
- required deduction uses hidden author knowledge;
- plausible alternate culprit satisfies all objective evidence;
- critical math, timing, access, spatial, or reveal constraints are inconsistent.

## Procedure

1. Simulate a player with only pre-accusation delivered material.
2. List every expected solution conclusion.
3. Map each conclusion to available evidence and permissible inference.
4. Verify red herrings are fact-compatible.
5. Verify exclusion information is available in time.
6. Check whether visuals accidentally provide unmodeled proof.
7. Distinguish difficulty from unfairness.
8. Record alternate solutions instead of explaining them away.

## Output

`Fair-play criterion | Evidence | Failure mode | Severity | Required owner | Re-test`

Conclude `FAIR-PLAY PASS` or `FAIL`.
