...
description: Maintains reusable regression setpoints for the active case so validated invariants survive later edits. Use after validated milestones, before broad refactors, and when deciding whether prior checks became stale.
---
...
A setpoint records an invariant/checkpoint derived from an authoritative or explicitly validated source. It is not new canon and does not upgrade source authority.

Possible setpoints:
- stable IDs;
- locked/user facts;
- critical time bounds;
- quantitative constraints;
- proof-limit statements;
- required evidence-to-deduction links;
- reveal ceilings;
- approved wording;
- passed-audit assumptions;
- visual Master/component versions and hashes.

## Procedure

1. Create/update a setpoint only from an identifiable source version.
2. Record source, version/hash if available, invariant, scope, owner, and validation state.
3. Before changes, find intersecting setpoints through the dependency graph.
4. Mark affected setpoints `STALE` until required re-tests pass.
5. Never preserve stale PASS because filenames/IDs stayed the same.
6. A0 decides how validated setpoints affect production-state governance.

## Output

`Setpoint ID | Invariant | Source/version | Scope | Current status | Change impact | Re-test owner | Disposition`

Statuses: `ACTIVE`, `STALE`, `SUPERSEDED`, `BLOCKED`.
