...
description: Protects document continuity in the active case across IDs, wording, facts, references, ownership, phase reveals, and approved player-facing content. Use before consolidating document additions, edits, deletions, or renames.
---
...
1. Inventory affected documents by stable DOC ID, filename, source, phase, owner, and status.
2. Preserve approved wording unless revision is authorized.
3. Check names, IDs, timestamps, quantities, locations, evidence references, and cross-document references against authority.
4. Trace incoming and outgoing references.
5. Check reveal ceilings and first-allowed phases.
6. Prevent internal production labels from entering diegetic content.
7. Use `evidence-dependency-auditor` for evidence-bearing documents.
8. Use chronology/alibi auditors when their domains change.
9. Include visible factual elements in visual-continuity impact.

## Output

`DOC ID | Change | Incoming refs | Outgoing refs | Facts touched | Evidence touched | Reveal impact | Visual impact | Required action`

Conclude `DOCUMENT CONTINUITY PASS` or `FAIL`.
